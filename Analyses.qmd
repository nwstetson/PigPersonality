...
# Don't have pig as row name
row.names(behavior_counts) <- 1:nrow(behavior_counts)
# Well fuck now it's removed the pig names entirely. I just want it to make the old row name into row 1 and make a new row name which is the row number

# Make wide version
behavior_counts_wide <-behavior_counts %>% 
  pivot_longer(cols = 2:20 , names_to = "Behavior", values_to = 'Occurances')
# Oops it's not separated by pig now because I accidentally made pig be a row label and not a row itself
```
...
```{r}
ggplot (obs, aes(x= reorder (Behavior, Behavior, table))) +
  geom_bar(color = "black", fill = "#0577B1") +
  theme_classic() +
  xlab("Behavior") +
  ylab("Count") +
  ggtitle("Behavior Across Pigs") +
  coord_flip() +
  facet_wrap (~ Pig)
```

🐽 It doesn't work if I try to use the same code as the scatter plot for survey scores, but modified for observations

```{r}
ggplot(obs, aes(x= reorder (Behavior, Behavior, table, group = Pig, color = Pig))) +
  geom_point() +
  geom_line(alpha=.5) +
    theme_classic() +
  xlab("Behavior") +
...
as.numeric(unlist(svy_items)) #force it to be numeric bc it thinks the NAs are like actual letters

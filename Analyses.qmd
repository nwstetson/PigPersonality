...
unorganized results version

```{r}
svy_wide <-svy_avg_m %>% 
  pivot_longer(cols = 14:48, names_to = "item.avg", values_to = 'item.avg.value')

svy_wide <- svy_m %>% 
  pivot_longer(cols = 50:61, names_to = "sub.facet.avg", values_to ='subfacet.value') 
...
```{r}
svy_f <- svy_avg_m %>% select(1:12,'Time.Known.D','Time.Known.T','Time.Known.Avg','SC1.D':'SC12.D','SC1.T':'SC12.T','SC1.Avg':'SC12.Avg') # All of them
```

...
ggplot(svy_wide, aes(x= facet, y= facet.value, group = PigName, color = PigName)) +
  ...
  ylab("Score") +
  ggtitle("Differences in Personality Scores Across Pigs") +
  guides(col = guide_legend(nrow = 10, title = "Pig"))
```

🐽 Jitter should be a better way to visualize it, but the points are stretched out in lines instead of dancing around and idk what's up with that
...
## Okay a box plot (I think this is probably the best way to visualize the scores)
...
ggplot(behavior_counts_wide, aes(x= Behavior, y= Count, group = PigName, fill = PigName)) +
  ...
  guides(fill = guide_legend(nrow = 10, title = "Pig"))
```
...
```{r}
ggplot(svy_items_corr_df, aes(x = svy_items_corr)) +
    geom_histogram(bins = 10, alpha = .5) +
    geom_vline(xintercept = mean(svy_items_corr)) +
...
ggplot(svy_wide, aes(x= sub.facet, y= subfacet.value, fill = Sex)) +
  geom_boxplot() +
  theme_classic() +
  scale_fill_manual(values = c("#faff75", "#9381e3")) +
  xlab("Personality Sub-Facet") +
  ylab("Score") +
  ggtitle("Variation in Personality Scores Across 20 Pigs")
```
...
disp_dominance$z(behavior_and_scores$Get.disp) <- (behavior_and_scores$Get.disp - mean(behavior_and_scores$Get.disp)) / sd(behavior_and_scores$Get.disp)

disp_dominance$z(behavior_and_scores$SC12) <- (behavior_and_scores$SC12 - mean(behavior_and_scores$SC12)) / sd(behavior_and_scores$SC12)

# Calculate the cross-product:  zx * zy
disp_dominance$z(behavior_and_scores$Get.disp)z(behavior_and_scores$SC12) <- disp_dominance$z(behavior_and_scores$Get.disp) * disp_dominance$z(behavior_and_scores$SC12)
...
```{r}
# Correlation between pig's SC12 vs # of times they got displaced by another pig
...
```{r}
# Is there a sex difference in personality scores?
ggplot(svy_wide, aes(x= sub.facet, y= subfacet.value, group = PigName, color = PigName)) +
  geom_boxplot(color = "navy", fill='lightsteelblue1') +
  theme_classic() +
  xlab("Personality Facet") +
  ylab("Score") +
  ggtitle("Differences in Personality Scores Accross Pigs") +
  facet_wrap (~ Sex) +
  guides(col = guide_legend(nrow = 10, title = "Pig"))
...
```{r}
ggplot(behavior_m, aes(x=Get.disp, fill=Background...8)) + #fix name in DF later
  theme_classic() +
  xlab("Get Displaced") +
  ylab("Counts") +

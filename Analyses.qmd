...
svy_by_rater <- read_csv("https://raw.githubusercontent.com/nwstetson/PigPersonality/main/svy_with_averages.csv", show_col_types = FALSE) # survey data (but with averages between raters)
...
svy_items <- svy %>% select(PigID, 4:38)
svy_items_avg <- svy_by_rater %>% select(PigID, 3:37) # all
# DF with just sub facet scores
pig_scores <- svy_m %>% select(PigID, 50:61)
svy_scores_avg <- svy_by_rater %>% select(PigID, 38:49) # all
```
...
```{r}
svy_tidy <-svy_avg_m %>%
  pivot_longer(cols = 14:48, names_to = "items.d", values_to ="items.d.value") 

svy_tidy <-svy_tidy %>%
  pivot_longer(cols = 38:49, names_to = "sub.facet.t", values_to ='subfacet.t.value') 
svy_tidy <-svy_tidy %>%
  pivot_longer(cols = 50:53, names_to = "facet.t", values_to ='facet.t.value') 
```
...
```{r}
svy_scores_avg$sub.facet <- factor(svy_avg_wide$sub.facet.avg, levels=rev(unique(svy_avg_wide$sub.facet.avg)))

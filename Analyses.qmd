...
I deleted the old code that made svy_wide smh

```{r}
svy_wide <-svy_m %>% 
  pivot_longer(cols = 14:48, names_to = "item", values_to = 'item.value')
svy_wide <- svy_wide %>% 
  pivot_longer(cols = 14:26, names_to = "sub.facet", values_to ='subfacet.value')
...
subfacets_tidy$subfacet.Avg <- factor(subfacets_tidy$subfacet.Avg, levels=rev(unique(subfacets_tidy$subfacet.Avg))) #adjust this to reorder tick marks
